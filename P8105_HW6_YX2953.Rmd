...
# Summarize results
results <- tibble(
  Model = c("Model 1 (Proposed)", "Model 2 (Length + Gestational Age)", "Model 3 (Interactions)"),
  RMSE = c(mean(rmse_model1), mean(rmse_model2), mean(rmse_model3))
)
...
summary_results <- rmse_results %>%
  group_by(Model) %>%
  summarize(Mean_RMSE = mean(RMSE)) %>%
  arrange(Mean_RMSE)

# Print summary results
summary_results |> knitr::kable(digits = 3)
